...
public class ItemLongSword extends ItemRPGSword
{

    public ItemLongSword(int par1, EnumToolMaterial material, String texture)
    {
        ...
        if (entityLiving != null)
        {
            if (getLevel(itemStack) >= 0)
            {
                itemStack.setItemDamage(itemStack.getItemDamage()
                        - entityLiving.getMaxHealth() / (getLevel(itemStack)-3));
            }
            else
            {
                itemStack.setItemDamage(itemStack.getItemDamage()
                            - entityLiving.getMaxHealth()-5);
            }
                ...
                if(player.inventory.hasItemStack(itemStack))
                {
                    if (player.inventory.getCurrentItem() == itemStack)
                    {
                        if(getLevel(itemStack) >= 5)
                            player.addPotionEffect(new PotionEffect(5, -1, ((getLevel(itemStack) / 3)-2)));
                        else{}
